...
class Polygon:
	...
	def get_minmax_points(self, width, height, padding=0):
		xl = min(self.points, key=lambda item: item[0])[0] - padding
		xr = max(self.points, key=lambda item: item[0])[0] + padding
		yt = max(self.points, key=lambda item: item[1])[1] + padding
		yb = min(self.points, key=lambda item: item[1])[1] - padding
